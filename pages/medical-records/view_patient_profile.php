<?php
// view_patient_profile.php - Patient Profile View for All Authorized Roles
...
set_error_handler(function($severity, $message, $file, $line) {
    error_log("Patient Profile View Error [$severity]: $message in $file on line $line");
    return true; // Don't execute PHP's internal error handler
});

// Set exception handler for uncaught exceptions
set_exception_handler(function($exception) {
    error_log("Patient Profile View Uncaught Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
// Validate patient ID
if (!$patient_id) {
    error_log("Patient profile view error: No patient ID provided");
...
if (!is_numeric($patient_id) || $patient_id <= 0) {
    error_log("Patient profile view error: Invalid patient ID format: " . $patient_id);
    header('HTTP/1.1 400 Bad Request');
...
// Get employee role for role-based permissions
// IMPORTANT: Patient Profile View is VIEW-ONLY for most roles, but Admin retains edit capabilities
$employee_role = $_SESSION['role'];
$canEdit = in_array(strtolower($employee_role), ['admin']); // Only admin can edit patient profiles
$canViewSensitive = true; // All authorized roles can view patient information for medical records purposes
...
try {
    ...
    foreach ($queries_to_try as $index => $query) {
        ...
        if (!$patient_row) {
            error_log("Patient profile view error: Patient ID $patient_id not found. Last error: " . ($last_error ?? 'No errors recorded'));
            header('HTTP/1.1 404 Not Found');
            die('Error: Patient record not found.');
        }

    } catch (Exception $e) {
        error_log("Error fetching patient data for profile view: " . $e->getMessage());
...
try {
    ...
    if ($stmt && $stmt->execute([$patient_id])) {
        ...
        error_log("Error fetching personal information for patient $patient_id in profile view: " . $e->getMessage());
    }try {
    ...
    if ($stmt && $stmt->execute([$patient_id])) {
        ...
        error_log("Error fetching emergency contact for patient $patient_id in profile view: " . $e->getMessage());
    }try {
    ...
    if (!$lifestyle_info) {
        ...
        error_log("Error fetching lifestyle info for patient $patient_id in profile view: " . $e->getMessage());
        $lifestyle_info = [];
...
try {
    if (!empty($patient['patient_id'])) {
        ...
        error_log("Error fetching vitals in profile view: " . $e->getMessage());
...
try {
    ...
    $latest_appointments = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        error_log("Error fetching appointments in profile view: " . $e->getMessage());
...
try {
    ...
    $latest_prescriptions = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        error_log("Error fetching prescriptions in profile view: " . $e->getMessage());
...
try {
    ...
    $lab_results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        error_log("Error fetching lab results in profile view: " . $e->getMessage());
...
try {
    ...
    $medical_history['immunizations'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        error_log("Error fetching medical history in profile view: " . $e->getMessage());
...
<head>
    ...
    <title>Patient Profile - <?= htmlspecialchars($patient['full_name']) ?> - WBHSMS</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <link rel="stylesheet" href="<?php echo $assets_path; ?>/css/sidebar.css">
    <link rel="stylesheet" href="<?php echo $assets_path; ?>/css/patient_profile.css" />
    <style>
        .homepage{
            margin-left: 300px;
        }
        ...
        /* Role-based View Mode - Hide editing elements for non-admin roles */
        .edit-btn,
        ...
        <div class="profile-layout" style="max-width: none;">
            ...
            <div class="profile-wrapper">
                ...
                ">
                    ...
                    "
                        src="<?= $patient_id
                                    ? (defined('WBHSMS_BASE_URL') ? WBHSMS_BASE_URL . '/vendor/photo_controller.php?patient_id=' . urlencode((string)$patient_id) : '../../vendor/photo_controller.php?patient_id=' . urlencode((string)$patient_id))
