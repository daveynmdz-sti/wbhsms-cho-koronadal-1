<?php
/**
 * Billing Station 4 Dashboard
 * 
 * Queue management dashboard for Billing Station 4
 * Features: Patient queue management, payment processing, visit completion
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from billing 4 dashboard');
...
// Station 4 Queue Management Functions
function getStation4Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation4Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation4QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation4QueueStatus Error: ' . $e->getMessage());
...
function getStation4Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation4Statistics Error: ' . $e->getMessage());
        return ['total_served' => 0, 'completed' => 0, 'waiting' => 0, 'in_progress' => 0, 'avg_wait_time' => 0];
    }
}

// =====================================================================
// BILLING STATION 4 CONFIGURATION
// =====================================================================

// Billing Station 4 Configuration
$STATION_CONFIG = [
    'station_id' => 4,                    
    'station_name' => 'Billing Station',  
    ...
    'allowed_roles' => ['admin', 'cashier', 'billing_clerk'], // Billing staff handle payments
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // Billing can complete visits as final step
...
// Get station details directly - Station 4 is always Billing
...
// Get current queue and statistics based on station_4_queue table
$currentQueue = getStation4Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation4Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
            ...
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'Patient called to billing station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to billing station successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        if ($result['success']) {
                            ...
                            $error = $result['message'] ?? 'Failed to complete billing process.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at billing station - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at billing station'
                        );
                        if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Billing completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            ...
                            $error = 'Failed to complete billing process before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at billing station'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at billing station'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        // Refresh data after action using station_4_queue
        $currentQueue = getStation4Queue($pdo);
        $stationStats = getStation4Statistics($pdo, $today);
        ...
        error_log('Billing station 4 dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'billing_clerk' => 'sidebar_cashier.php'  // Use cashier sidebar for billing clerk
];
...
<head>
    ...
    <style>
        /* Billing Station 4 Styles - Red Theme */
        :root {
            --station-primary: #dc2626;
            --station-secondary: #b91c1c;
            --station-accent: #fca5a5;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'billing_clerk' || $user_role === 'cashier' ? 'cashier' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                ...
                | Cashier: <?php echo htmlspecialchars($_SESSION['employee_name'] ?? 'Unknown'); ?>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <h3>₱<?php echo number_format($stationStats['total_amount'] ?? 0, 2); ?></h3>
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-receipt"></i>
                    Current Patient - Billing & Payment
                </div>
            ...
            <div class="current-patient-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete billing process?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for billing and payment.</p>
        ...
        <div class="queue-management-grid">
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Payment'); ?></p>
                                    </div>
            ...
            <div class="queue-section in-progress-queue">
                <div class="queue-header">
                    <h3>
                        <i class="fas fa-user-check"></i>
                        Being Served (<?php echo count($inProgressQueue); ?> patients)
                    </h3>
                </div>
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">At Billing</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                        ...
                                        <div class="action-group">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-success btn-sm" 
                                                        onclick="return confirm('Complete billing?')">
                        ...
                        <div class="patient-cards">
                            <?php foreach (array_slice($completedQueue, -10) as $patient): // Show last 10 completed ?>
                                <div class="patient-card completed" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-completed">Paid</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
