<?php
/**
 * TB DOTS Station 8 Dashboard
 * 
 * Queue management dashboard for TB DOTS Station 8
 * Features: Patient queue management, TB treatment monitoring, medication tracking
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from TB DOTS 8 dashboard');
...
// Station 8 Queue Management Functions
function getStation8Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation8Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation8QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation8QueueStatus Error: ' . $e->getMessage());
...
function getStation8Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation8Statistics Error: ' . $e->getMessage());
        return ['total_served' => 0, 'completed' => 0, 'waiting' => 0, 'in_progress' => 0, 'avg_wait_time' => 0];
    }
}

// =====================================================================
// TB DOTS STATION 8 CONFIGURATION
// =====================================================================

// TB DOTS Station 8 Configuration
$STATION_CONFIG = [
    'station_id' => 8,                    
    'station_name' => 'TB DOTS Station',  
    'station_type' => 'tb_dots',          
    'icon' => 'fa-lungs',                 
    'color_scheme' => [
    ...
    'allowed_roles' => ['admin', 'doctor', 'nurse', 'tb_coordinator'], // TB treatment specialists
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // TB DOTS can complete visits for routine monitoring
...
$station = [
    ...
    'station_code' => 'TB_DOTS_8',
    'description' => 'TB DOTS Treatment and Monitoring'
];
...
$currentQueue = getStation8Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation8Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation8QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to TB DOTS station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to TB DOTS station successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation8QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'TB DOTS monitoring completed'
                        );
                        if ($result['success']) {
                            $message = 'TB DOTS monitoring completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete TB DOTS monitoring.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation8QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at TB DOTS station - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation8QueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at TB DOTS station'
                        );
                        if ($result['success']) {
            ...
            case 'transfer_to_consultation2':
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = updateStation8QueueStatus(
                            $pdo,
                            $patient_queue['queue_entry_id'], 
                            'completed', 
                            $_SESSION['employee_id'],
                            'TB DOTS monitoring completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            $transfer_result = addToStationQueue(
                                ...
                                $_SESSION['username'] ?? 'TB_DOTS_STAFF',
                            ...
                            $error = 'Failed to complete TB DOTS monitoring before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = updateStation8QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at TB DOTS station'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = updateStation8QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at TB DOTS station'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('TB DOTS station 8 dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'tb_coordinator' => 'sidebar_nurse.php'  // Use nurse sidebar for TB coordinator
];
...
<head>
    ...
    <style>
        /* TB DOTS Station 8 Styles - Amber Theme */
        :root {
            --station-primary: #f59e0b;
            --station-secondary: #d97706;
            --station-accent: #fde68a;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'tb_coordinator' ? 'nurse' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Tuberculosis Treatment Monitoring & DOTS Implementation
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-lungs"></i>
                    Current Patient - TB DOTS Monitoring
                </div>
            ...
            <div class="current-patient-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete TB DOTS monitoring?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for TB DOTS monitoring.</p>
        ...
        <div class="queue-management-grid">
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'TB DOTS'); ?></p>
                                    </div>
            ...
            <div class="queue-section in-progress-queue">
                <div class="queue-header">
                    <h3>
                        <i class="fas fa-user-check"></i>
                        Being Monitored (<?php echo count($inProgressQueue); ?> patients)
                    </h3>
                </div>
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">TB DOTS</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                        ...
                                        <div class="action-group">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-success btn-sm" 
                                                        onclick="return confirm('Complete monitoring?')">
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach (array_slice($completedQueue, -10) as $patient): // Show last 10 completed ?>
                                <div class="patient-card completed" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-completed">Monitored</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No TB monitoring completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
