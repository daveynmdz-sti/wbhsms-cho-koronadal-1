<?php
// view_patient_profile.php - DHO Patient Profile View
...
set_error_handler(function($severity, $message, $file, $line) {
    error_log("DHO Patient Profile View Error [$severity]: $message in $file on line $line");
    return true; // Don't execute PHP's internal error handler
});

// Set exception handler for uncaught exceptions
set_exception_handler(function($exception) {
    error_log("DHO Patient Profile View Uncaught Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
// Validate patient ID
if (!$patient_id) {
    error_log("DHO patient profile view error: No patient ID provided");
...
if (!is_numeric($patient_id) || $patient_id <= 0) {
    error_log("DHO patient profile view error: Invalid patient ID format: " . $patient_id);
    header('HTTP/1.1 400 Bad Request');
...
// Verify DHO is logged in - use session system
require_employee_login();

// Check if user has permission to view patient records - DHO and BHW only
$authorized_roles = ['dho', 'bhw'];
...
try {
    ...
    foreach ($queries_to_try as $index => $query) {
        try {
            ...
            error_log("DHO patient profile view query error: " . $last_error);
            continue;
        }
    }

    if (!$patient_row) {
        error_log("DHO patient profile view error: Patient ID $patient_id not found. Last error: " . ($last_error ?? 'No errors recorded'));
        header('HTTP/1.1 404 Not Found');
        die('Error: Patient record not found.');
    }

} catch (Exception $e) {
    error_log("Error fetching patient data for DHO view: " . $e->getMessage());
...
try {
    ...
    error_log("Error fetching personal information for patient $patient_id in DHO view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching emergency contact for patient $patient_id in DHO view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lifestyle info for patient $patient_id in DHO view: " . $e->getMessage());
    $lifestyle_info = [];
}
...
try {
    ...
    error_log("Error fetching vitals in DHO view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching appointments in DHO view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching prescriptions in DHO view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lab results in DHO view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching medical history in DHO view: " . $e->getMessage());
}
...
<head>
    ...
    <title>DHO View - Patient Profile - <?= htmlspecialchars($patient['full_name']) ?> - WBHSMS</title>
    ...
    <style>
        ...
        /* DHO View Specific Styles - Hide editing elements */
        .edit-btn,
...
<body>
    ...
    include $root_path . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'sidebar_dho.php';
    ?>
    ...
    <section class="homepage">
        <div class="profile-heading-bar"
            ...
            <h1 style="margin:0;font-size:2.2em;letter-spacing:1px;">
                <i class="fas fa-building"></i> PATIENT PROFILE <span style="font-size: 0.65em; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 20px; vertical-align: middle;">DHO VIEW</span>
            </h1>
        ...
        <!-- DHO View: Patient Summary Header -->
    ...
    <script>
        ...
        // Enhanced print functionality for DHO view
