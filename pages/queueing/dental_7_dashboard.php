<?php
/**
 * Dental Station 7 Dashboard
 * 
 * Queue management dashboard for Dental Station 7
 * Features: Patient queue management, dental consultation tracking, specialized dental workflows
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from dental 7 dashboard');
...
// Station 7 Queue Management Functions
function getStation7Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation7Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation7QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation7QueueStatus Error: ' . $e->getMessage());
...
function getStation7Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation7Statistics Error: ' . $e->getMessage());
        return ['total_served' => 0, 'completed' => 0, 'waiting' => 0, 'in_progress' => 0, 'avg_wait_time' => 0];
    }
}

// =====================================================================
// DENTAL STATION 7 CONFIGURATION
// =====================================================================

// Dental Station 7 Configuration
$STATION_CONFIG = [
    'station_id' => 7,                    
    'station_name' => 'Dental Station',   
    'station_type' => 'dental',           
    'icon' => 'fa-tooth',                 
    'color_scheme' => [
    ...
    'allowed_roles' => ['admin', 'doctor', 'dentist'], // Doctors and dentists handle dental care
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // Dental station can complete visits for simple cases
...
// Get station details directly - Station 7 is always Dental
...
// Get current queue and statistics based on station_7_queue table
$currentQueue = getStation7Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation7Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
            ...
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'Patient called to dental station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to dental station successfully.';
                    } else {
            ...
            case 'complete_current':
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        if ($result['success']) {
                            $message = 'Dental consultation completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete dental consultation.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at dental station - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at dental station'
                        );
                        if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Dental consultation completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            ...
                            $error = 'Failed to complete dental consultation before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at dental station'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at dental station'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        // Refresh data after action using station_7_queue
        $currentQueue = getStation7Queue($pdo);
        $stationStats = getStation7Statistics($pdo, $today);
        ...
        error_log('Dental station 7 dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'dentist' => 'sidebar_doctor.php' // Use doctor sidebar for dentist
];
...
<head>
    ...
    <style>
        /* Dental Station 7 Styles - Sky Blue Theme */
        :root {
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'dentist' ? 'doctor' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Dental Consultation & Oral Health Services
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <p>Dental Consultations Today</p>
            </div>
            <div class="stat-card">
                <div class="icon">
                    <i class="fas fa-clock"></i>
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-tooth"></i>
                    Current Patient - Dental Consultation
                </div>
            ...
            <div class="current-patient-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete dental consultation?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for dental consultation.</p>
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Dental'); ?></p>
                                    </div>
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">At Dental</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No dental consultations completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
