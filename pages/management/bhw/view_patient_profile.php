<?php
// view_patient_profile.php - BHW Patient Profile View
...
set_error_handler(function($severity, $message, $file, $line) {
    error_log("BHW Patient Profile View Error [$severity]: $message in $file on line $line");
    return true; // Don't execute PHP's internal error handler
});

// Set exception handler for uncaught exceptions
set_exception_handler(function($exception) {
    error_log("BHW Patient Profile View Uncaught Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
// Validate patient ID
if (!$patient_id) {
    error_log("BHW patient profile view error: No patient ID provided");
...
if (!is_numeric($patient_id) || $patient_id <= 0) {
    error_log("BHW patient profile view error: Invalid patient ID format: " . $patient_id);
    header('HTTP/1.1 400 Bad Request');
...
// Verify BHW is logged in - use session system
require_employee_login();

// Check if user has permission to view patient records - BHW and DHO only
$authorized_roles = ['bhw', 'dho'];
require_employee_role($authorized_roles);

// BHW Barangay-based Access Control Security Check
if ($_SESSION['role'] === 'bhw') {
    try {
        ...
        $bhw_barangay_stmt = $pdo->prepare($bhw_barangay_sql);
        if (!$bhw_barangay_stmt) {
            error_log("BHW barangay check preparation failed: " . implode(' ', $pdo->errorInfo()));
            die('Error: Access validation failed.');
        }
        
        $bhw_barangay_stmt->execute([$_SESSION['employee_id']]);
        $bhw_barangay_result = $bhw_barangay_stmt->fetch(PDO::FETCH_ASSOC);

        if (!$bhw_barangay_result) {
            error_log("BHW access denied: No facility assignment found for employee ID " . $_SESSION['employee_id']);
            header('HTTP/1.1 403 Forbidden');
            die('Error: Access denied - No facility assignment found.');
        }

        $bhw_barangay = $bhw_barangay_result['barangay_id'];

        // Verify that the requested patient belongs to the BHW's barangay
        $patient_barangay_sql = "SELECT barangay_id FROM patients WHERE patient_id = ?";
        $patient_barangay_stmt = $pdo->prepare($patient_barangay_sql);
        if (!$patient_barangay_stmt) {
            error_log("Patient barangay check preparation failed: " . implode(' ', $pdo->errorInfo()));
            die('Error: Patient validation failed.');
        }

        $patient_barangay_stmt->execute([$patient_id]);
        $patient_barangay_result = $patient_barangay_stmt->fetch(PDO::FETCH_ASSOC);

        if (!$patient_barangay_result) {
            error_log("BHW access denied: Patient ID $patient_id not found");
            header('HTTP/1.1 404 Not Found');
            die('Error: Patient record not found.');
        }

        $patient_barangay = $patient_barangay_result['barangay_id'];

        // Check if patient belongs to BHW's barangay
        if ($patient_barangay !== $bhw_barangay) {
            error_log("BHW access denied: Employee ID " . $_SESSION['employee_id'] . " (barangay $bhw_barangay) attempted to access patient ID $patient_id (barangay $patient_barangay)");
            header('HTTP/1.1 403 Forbidden');
            die('Error: Access denied - Patient is outside your barangay jurisdiction.');
        }

        // Log successful access for audit purposes
        error_log("BHW access granted: Employee ID " . $_SESSION['employee_id'] . " accessing patient ID $patient_id within barangay $bhw_barangay");

    } catch (Exception $e) {
        error_log("BHW barangay access control error: " . $e->getMessage());
        header('HTTP/1.1 500 Internal Server Error');
        die('Error: Access validation failed. Please try again later.');
    }
}

// DHO District-based Access Control Security Check  
if ($_SESSION['role'] === 'dho') {
    try {
        ...
        $dho_district_stmt = $pdo->prepare($dho_district_sql);
        if (!$dho_district_stmt) {
            error_log("DHO district check preparation failed: " . implode(' ', $pdo->errorInfo()));
            die('Error: Access validation failed.');
        }
        
        $dho_district_stmt->execute([$_SESSION['employee_id']]);
        $dho_district_result = $dho_district_stmt->fetch(PDO::FETCH_ASSOC);

        if (!$dho_district_result) {
            error_log("DHO access denied: No facility assignment found for employee ID " . $_SESSION['employee_id']);
            header('HTTP/1.1 403 Forbidden');
            die('Error: Access denied - No facility assignment found.');
        }

        $dho_district = $dho_district_result['district_id'];

        // Verify that the requested patient belongs to the DHO's district
        $patient_district_sql = "SELECT b.district_id 
                                 FROM patients p 
                                 LEFT JOIN barangay b ON p.barangay_id = b.barangay_id 
                                 WHERE p.patient_id = ?";
        $patient_district_stmt = $pdo->prepare($patient_district_sql);
        if (!$patient_district_stmt) {
            error_log("Patient district check preparation failed: " . implode(' ', $pdo->errorInfo()));
            die('Error: Patient validation failed.');
        }

        $patient_district_stmt->execute([$patient_id]);
        $patient_district_result = $patient_district_stmt->fetch(PDO::FETCH_ASSOC);

        if (!$patient_district_result) {
            error_log("DHO access denied: Patient ID $patient_id not found or has no barangay assignment");
            header('HTTP/1.1 404 Not Found');
            die('Error: Patient record not found.');
        }

        $patient_district = $patient_district_result['district_id'];

        // Check if patient belongs to DHO's district
        if ($patient_district !== $dho_district) {
            error_log("DHO access denied: Employee ID " . $_SESSION['employee_id'] . " (district $dho_district) attempted to access patient ID $patient_id (district $patient_district)");
            header('HTTP/1.1 403 Forbidden');
            die('Error: Access denied - Patient is outside your district jurisdiction.');
        }

        // Log successful access for audit purposes
        error_log("DHO access granted: Employee ID " . $_SESSION['employee_id'] . " accessing patient ID $patient_id within district $dho_district");

    } catch (Exception $e) {
        error_log("DHO district access control error: " . $e->getMessage());
        header('HTTP/1.1 500 Internal Server Error');
        die('Error: Access validation failed. Please try again later.');
    }
}
...
try {
    ...
    foreach ($queries_to_try as $index => $query) {
        try {
            ...
            error_log("Doctor patient profile view query error: " . $last_error);
            continue;
        }
    }

    if (!$patient_row) {
        error_log("Doctor patient profile view error: Patient ID $patient_id not found. Last error: " . ($last_error ?? 'No errors recorded'));
        header('HTTP/1.1 404 Not Found');
        die('Error: Patient record not found.');
    }

} catch (Exception $e) {
    error_log("Error fetching patient data for BHW view: " . $e->getMessage());
...
try {
    ...
    error_log("Error fetching personal information for patient $patient_id in BHW view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching emergency contact for patient $patient_id in BHW view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lifestyle info for patient $patient_id in BHW view: " . $e->getMessage());
    $lifestyle_info = [];
}
...
try {
    ...
    error_log("Error fetching vitals in BHW view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching appointments in BHW view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching prescriptions in BHW view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lab results in BHW view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching medical history in BHW view: " . $e->getMessage());
}
...
<head>
    ...
    <title>BHW View - Patient Profile - <?= htmlspecialchars($patient['full_name']) ?> - WBHSMS</title>
    ...
    <style>
        ...
        /* BHW View Specific Styles - Hide editing elements */
        .edit-btn,
...
<body>
    <?php
    $activePage = 'patients';
    include $root_path . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'sidebar_bhw.php';
    ?>
    ...
    <section class="homepage">
        <div class="profile-heading-bar"
            ...
            <h1 style="margin:0;font-size:2.2em;letter-spacing:1px;">
                <i class="fas fa-users"></i> PATIENT PROFILE <span style="font-size: 0.65em; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 20px; vertical-align: middle;">BHW VIEW</span>
            </h1>
        ...
        <!-- BHW View: Patient Summary Header -->
    ...
    <script>
        ...
        // Enhanced print functionality for BHW view
