<?php
/**
 * Animal Bite Station 11 Dashboard
 * 
 * Queue management dashboard for Animal Bite Station 11
 * Features: Patient queue management, animal bite treatment, rabies vaccination, wound care
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from Animal Bite 11 dashboard');
...
// =====================================================================
// ANIMAL BITE STATION 11 CONFIGURATION
// =====================================================================

// Animal Bite Station 11 Configuration
$STATION_CONFIG = [
    'station_id' => 11,                   
    'station_name' => 'Animal Bite Treatment Station',  
    'station_type' => 'animal_bite',              
    'icon' => 'fa-dog',                           
    'color_scheme' => [
    ...
    'allowed_roles' => ['admin', 'doctor', 'nurse', 'emergency_tech'], // Emergency care specialists
    'next_stations' => [                  
        'pharmacy' => ['station_id' => 14, 'name' => 'Pharmacy 1', 'icon' => 'fa-pills'],
        'pharmacy2' => ['station_id' => 15, 'name' => 'Pharmacy 2', 'icon' => 'fa-capsules'],
        'vaccination' => ['station_id' => 9, 'name' => 'Vaccination Station', 'icon' => 'fa-syringe'],
        'laboratory' => ['station_id' => 13, 'name' => 'Laboratory', 'icon' => 'fa-microscope'],
        ...
        'document' => ['station_id' => 12, 'name' => 'Document Requests', 'icon' => 'fa-file-alt']
    ],
    'can_complete_visit' => true,         // Animal bite station can complete visits for emergency care
...
// Station 11 Queue Management Functions
function getStation11Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation11Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation11QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation11QueueStatus Error: ' . $e->getMessage());
...
function getStation11Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation11Statistics Error: ' . $e->getMessage());
...
// Get station details - we don't need this anymore since we're using individual tables
$station_id = $STATION_CONFIG['station_id'];

// Get current queue and statistics using station-specific functions
$currentQueue = getStation11Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation11Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation11QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to animal bite treatment station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to animal bite treatment station successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation11QueueStatus(
                            ...
                            'done', 
                            $_SESSION['employee_id'],
                            'Animal bite treatment completed'
                        );
                        if ($result['success']) {
                            $message = 'Animal bite treatment completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete animal bite treatment.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation11QueueStatus(
                            ...
                            'done', 
                            $_SESSION['employee_id'],
                            'Visit completed at animal bite treatment station - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation11QueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at animal bite treatment station'
                        );
                        if ($result['success']) {
            ...
            case 'transfer_to_document':
                $transfer_map = [
                    ...
                    'transfer_to_document' => ['id' => 12, 'name' => 'Document Requests', 'type' => 'document']
                ];
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = updateStation11QueueStatus(
                            $pdo,
                            $patient_queue['queue_entry_id'], 
                            'done', 
                            $_SESSION['employee_id'],
                            'Animal bite treatment completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            $transfer_result = addToStationQueue(
                                ...
                                $patient_queue['patient_username'],
                            ...
                            $error = 'Failed to complete animal bite treatment before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = updateStation11QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at animal bite treatment station'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = updateStation11QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at animal bite treatment station'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('Animal bite station 11 dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'emergency_tech' => 'sidebar_nurse.php'  // Use nurse sidebar for emergency tech
];
...
<head>
    ...
    <style>
        /* Animal Bite Station 11 Styles - Teal Theme */
        :root {
            --station-primary: #0d9488;
            --station-secondary: #0f766e;
            --station-accent: #99f6e4;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'emergency_tech' ? 'nurse' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Emergency Animal Bite Treatment, Rabies Vaccination & Wound Care Management
            ...
            <div class="station-status">
                ...
                <div class="status-badge">
                    ...
                    <?php echo htmlspecialchars($_SESSION['employee_name'] ?? 'Unknown'); ?>
                </div>
            </div>
        </div>
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-dog"></i>
                    Current Patient - Animal Bite Treatment
                </div>
            ...
            <div class="current-patient-content">
                <?php if ($currentPatient): ?>
                    <div class="patient-info">
                        ...
                        <div class="patient-detail">
                            ...
                            <div>
                                <span class="priority-badge priority-<?php echo strtolower($currentPatient['priority_level'] ?? 'emergency'); ?>">
                                    <?php echo ucfirst($currentPatient['priority_level'] ?? 'Emergency'); ?>
                                </span>
                            </div>
                        </div>
                        <div class="patient-detail">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete animal bite treatment?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for animal bite treatment.</p>
        ...
        <div class="queue-management-grid">
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        ...
                                        <span class="priority-badge priority-<?php echo strtolower($patient['priority_level'] ?? 'emergency'); ?>">
                                            <?php echo ucfirst($patient['priority_level'] ?? 'Emergency'); ?>
                                        </span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                        <p>Wait: <?php echo $patient['wait_time_minutes'] ?? 0; ?> min</p>
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Animal Bite'); ?></p>
                                    </div>
            ...
            <div class="queue-section in-progress-queue">
                <div class="queue-header">
                    <h3>
                        <i class="fas fa-user-check"></i>
                        Being Treated (<?php echo count($inProgressQueue); ?> patients)
                    </h3>
                </div>
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">Animal Bite</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                        ...
                                        <div class="action-group">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-success btn-sm" 
                                                        onclick="return confirm('Complete treatment?')">
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach (array_slice($completedQueue, -10) as $patient): // Show last 10 completed ?>
                                <div class="patient-card completed" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-completed">Treated</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No animal bite cases completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
