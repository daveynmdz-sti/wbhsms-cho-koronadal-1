<?php
/**
 * Pharmacy Station 14 Dashboard
 * 
 * Queue management dashboard for Pharmacy Station 14
 * Features: Patient queue management, medication dispensing tracking, visit completion
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from pharmacy 14 dashboard');
...
// Station 14 Queue Management Functions
function getStation14Queue($pdo) {
    $stmt = $pdo->prepare("
        SELECT 
...
function updateStation14QueueStatus($pdo, $queue_entry_id, $status, $employee_id, $notes = null) {
    $stmt = $pdo->prepare("
        UPDATE station_14_queue 
...
function addToStationQueue($pdo, $patient_id, $username, $visit_id, $appointment_id, $service_id, $queue_type, $station_id, $priority_level = 'normal') {
    $stmt = $pdo->prepare("
        INSERT INTO station_14_queue (patient_id, username, visit_id, appointment_id, service_id, queue_type, station_id, priority_level, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'waiting', NOW())
    ");
...
function getStation14Statistics($pdo, $date) {
    $stmt = $pdo->prepare("
        SELECT 
...
// =====================================================================
// PHARMACY STATION 14 CONFIGURATION
// =====================================================================

// Pharmacy Station 14 Configuration
$STATION_CONFIG = [
    'station_id' => 14,                    
    'station_name' => 'Pharmacy 1',        
    ...
    'allowed_roles' => ['admin', 'pharmacist'], // Pharmacists handle medication dispensing
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // Pharmacy can complete visits as final step
...
$station = [
    ...
    'station_code' => 'PHARMACY_14',
    'description' => 'Primary Pharmacy - Medication Dispensing'
];
...
$currentQueue = getStation14Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation14Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation14QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to pharmacy 1'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to pharmacy successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation14QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Medication dispensing completed at pharmacy 1'
                        );
                        if ($result['success']) {
                            $message = 'Medication dispensing completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete medication dispensing.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation14QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at pharmacy 1 - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Pharmacy services completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            ...
                            $error = 'Failed to complete pharmacy services before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at pharmacy 1'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at pharmacy 1'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('Pharmacy station 14 dashboard action error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        /* Pharmacy Station 14 Styles - Green Theme */
        :root {
            --station-primary: #16a34a;
            --station-secondary: #15803d;
            --station-accent: #86efac;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'pharmacist' ? 'pharmacist' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Medication Dispensing & Queue Management
                | Pharmacist: <?php echo htmlspecialchars($_SESSION['employee_name'] ?? 'Unknown'); ?>
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-pills"></i>
                    Current Patient - Pharmacy Services
                </div>
            ...
            <div class="current-patient-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete medication dispensing?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for medication dispensing.</p>
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Prescription'); ?></p>
                                    </div>
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">At Pharmacy</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                        ...
                                        <div class="action-group">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-success btn-sm" 
                                                        onclick="return confirm('Complete dispensing?')">
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No prescriptions completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
