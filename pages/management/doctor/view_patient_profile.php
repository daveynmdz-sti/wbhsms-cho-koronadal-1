<?php
// view_patient_profile.php - Doctor Patient Profile View
...
set_error_handler(function($severity, $message, $file, $line) {
    error_log("Doctor Patient Profile View Error [$severity]: $message in $file on line $line");
    return true; // Don't execute PHP's internal error handler
});

// Set exception handler for uncaught exceptions
set_exception_handler(function($exception) {
    error_log("Doctor Patient Profile View Uncaught Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
// Validate patient ID
if (!$patient_id) {
    error_log("Doctor patient profile view error: No patient ID provided");
...
if (!is_numeric($patient_id) || $patient_id <= 0) {
    error_log("Doctor patient profile view error: Invalid patient ID format: " . $patient_id);
    header('HTTP/1.1 400 Bad Request');
...
// Verify doctor is logged in - use session system
require_employee_login();

// Check if user has permission to view patient records - doctors only (and nurses if needed)
...
try {
    ...
    error_log("Error fetching patient data for doctor view: " . $e->getMessage());
...
try {
    ...
    error_log("Error fetching personal information for patient $patient_id in doctor view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching emergency contact for patient $patient_id in doctor view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lifestyle info for patient $patient_id in doctor view: " . $e->getMessage());
    $lifestyle_info = [];
}
...
try {
    ...
    error_log("Error fetching vitals in doctor view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching appointments in doctor view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching prescriptions in doctor view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lab results in doctor view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching medical history in doctor view: " . $e->getMessage());
}
...
<head>
    <meta charset="UTF-8" />
    <title>Doctor View - Patient Profile - <?= htmlspecialchars($patient['full_name']) ?> - WBHSMS</title>
    ...
    <style>
        ...
        /* Doctor View Specific Styles - Hide editing elements */
        .edit-btn,
...
<body>
    <?php
    $activePage = 'patients';
    include $root_path . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'sidebar_doctor.php';
    ?>
    ...
    <section class="homepage">
        <div class="profile-heading-bar"
            ...
            <h1 style="margin:0;font-size:2.2em;letter-spacing:1px;">
                <i class="fas fa-user-md"></i> PATIENT PROFILE <span style="font-size: 0.65em; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 20px; vertical-align: middle;">DOCTOR VIEW</span>
            </h1>
        ...
        <!-- Doctor View: Patient Summary Header -->
    ...
    <script>
        ...
        // Enhanced print functionality for doctor view
