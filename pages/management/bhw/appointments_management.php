<?php
// appointments_management.php - BHW Side
// Role-specific appointments management for BHW (Barangay Health Worker)
...
// Get employee facility assignment and barangay for jurisdiction restrictions
$employee_facility_query = "SELECT f.barangay_id 
                           FROM employees e 
...
$employee_barangay_id = $employee_facility['barangay_id'] ?? null;

// BHW role restrictions: Must have barangay assignment
if ($employee_role === 'bhw' && !$employee_barangay_id) {
    if (ob_get_level()) {
        ob_end_clean();
    }
    header('Location: dashboard.php?error=no_barangay_assignment');
    exit();
}
        ...
        if (empty($cancel_reason) || empty($employee_password)) {
            ...
            try {
                ...
                if (!$employee || !password_verify($employee_password, $employee['password'])) {
                    ...
                    if ($employee_role === 'bhw' && $employee_barangay_id) {
                        // BHW can only cancel appointments for patients in their barangay
                        $barangay_check_stmt = $conn->prepare("SELECT COUNT(*) as count FROM appointments a 
                                                              JOIN patients p ON a.patient_id = p.patient_id 
                                                              WHERE a.appointment_id = ? AND p.barangay_id = ?");
                        $barangay_check_stmt->bind_param("ii", $appointment_id, $employee_barangay_id);
                        $barangay_check_stmt->execute();
                        $barangay_result = $barangay_check_stmt->get_result();
                        $barangay_data = $barangay_result->fetch_assoc();
                        $barangay_check_stmt->close();
                        
                        if ($barangay_data['count'] == 0) {
                            $can_cancel = false;
                            $jurisdiction_error = "Access denied: You can only manage appointments for patients within your assigned barangay.";
                        }
                    }
...
try {
    ...
    if ($employee_role === 'bhw' && $employee_barangay_id) {
        $stats_where_conditions[] = "p.barangay_id = ?";
        $stats_params[] = $employee_barangay_id;
        $stats_types .= 'i';
    }

    $stats_where = !empty($stats_where_conditions) ? 'WHERE ' . implode(' AND ', $stats_where_conditions) : '';

    $stmt = $conn->prepare("SELECT status, COUNT(*) as count FROM appointments a LEFT JOIN facilities f ON a.facility_id = f.facility_id LEFT JOIN patients p ON a.patient_id = p.patient_id $stats_where GROUP BY status");
...
try {
    if ($employee_role === 'bhw' && $employee_barangay_id) {
        // BHW: Filter facilities by barangay
        $stmt = $conn->prepare("SELECT facility_id, name, district FROM facilities WHERE status = 'active' AND barangay_id = ? ORDER BY name ASC");
...
<head>
    ...
    <title>CHO Koronadal — BHW Appointments Management</title>
...
<body>
    <div class="homepage">
        ...
        <div class="page-header">
            <h1><i class="fas fa-calendar-alt"></i> BHW - Appointments Management</h1>
        </div>
