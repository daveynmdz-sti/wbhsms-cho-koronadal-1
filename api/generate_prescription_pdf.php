<?php
// Include database and vendor autoload
...
// Enhanced session handling - start session first
...
if ($debug_mode) {
    error_log("PDF API Session Debug - Session ID: " . session_id());
    error_log("PDF API Session Debug - Patient ID: " . (isset($_SESSION['patient_id']) ? $_SESSION['patient_id'] : 'NOT SET'));
    error_log("PDF API Session Debug - Employee ID: " . (isset($_SESSION['employee_id']) ? $_SESSION['employee_id'] : 'NOT SET'));
}

// Check authentication - multiple approaches
$is_patient = false;
$is_employee = false;
$patient_id = null;
...
} elseif (isset($_SESSION['employee_id']) && !empty($_SESSION['employee_id'])) {
    $is_employee = true;
}

// If no active session found, try loading session configs
if (!$is_patient && !$is_employee) {
    // Try patient session config
    try {
        ...
        // Ignore patient session errors
    }
    
    // Try employee session config if still not authenticated
    if (!$is_patient) {
        try {
            ...
            // Ignore employee session errors
        }
    }
}
...
if (!$is_patient && !$is_employee) {
    ...
    echo json_encode(['success' => false, 'message' => 'Unauthorized access - no valid session found']);
    exit();
}
...
try {
    ...
    // Add patient restriction if accessing as patient
    if ($is_patient) {
    ...
    if ($pharmStmt) {
        try {
            ...
            // Continue without pharmacist info if table doesn't exist
        }
    }
