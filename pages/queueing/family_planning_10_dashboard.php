<?php
/**
 * Family Planning Station 10 Dashboard
 * 
 * Queue management dashboard for Family Planning Station 10
 * Features: Patient queue management, contraceptive counseling, reproductive health services
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from Family Planning 10 dashboard');
...
// Station 10 Queue Management Functions
function getStation10Queue($pdo) {
    $stmt = $pdo->prepare("
        SELECT 
...
function updateStation10QueueStatus($pdo, $queue_entry_id, $status, $employee_id, $notes = null) {
    $stmt = $pdo->prepare("
        UPDATE station_10_queue 
...
function addToStationQueue($pdo, $patient_id, $username, $visit_id, $appointment_id, $service_id, $queue_type, $station_id, $priority_level = 'normal') {
    $stmt = $pdo->prepare("
        INSERT INTO station_10_queue (patient_id, username, visit_id, appointment_id, service_id, queue_type, station_id, priority_level, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'waiting', NOW())
    ");
...
function getStation10Statistics($pdo, $date) {
    $stmt = $pdo->prepare("
        SELECT 
...
// =====================================================================
// FAMILY PLANNING STATION 10 CONFIGURATION
// =====================================================================

// Family Planning Station 10 Configuration
$STATION_CONFIG = [
    'station_id' => 10,                   
    'station_name' => 'Family Planning Station',  
    'station_type' => 'family_planning',          
    'icon' => 'fa-heart',                         
    'color_scheme' => [
    ...
    'allowed_roles' => ['admin', 'doctor', 'nurse', 'midwife', 'fp_coordinator'], // FP specialists
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // FP can complete visits for contraceptive services
...
$station = [
    ...
    'station_code' => 'FAMILY_PLANNING_10',
...
$currentQueue = getStation10Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation10Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation10QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to family planning station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to family planning station successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation10QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Family planning consultation completed'
                        );
                        if ($result['success']) {
                            $message = 'Family planning consultation completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete family planning consultation.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation10QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at family planning station - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation10QueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at family planning station'
                        );
                        if ($result['success']) {
            ...
            case 'transfer_to_document':
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = updateStation10QueueStatus(
                            $pdo,
                            $patient_queue['queue_entry_id'], 
                            'completed', 
                            $_SESSION['employee_id'],
                            'Family planning consultation completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            ...
                            $error = 'Failed to complete family planning consultation before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = updateStation10QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at family planning station'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = updateStation10QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at family planning station'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('Family planning station 10 dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'midwife' => 'sidebar_nurse.php',  // Use nurse sidebar for midwife
    'fp_coordinator' => 'sidebar_nurse.php'  // Use nurse sidebar for FP coordinator
];
...
<head>
    ...
    <style>
        /* Family Planning Station 10 Styles - Pink Theme */
        :root {
            --station-primary: #ec4899;
            --station-secondary: #be185d;
            --station-accent: #fbcfe8;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'midwife' || $user_role === 'fp_coordinator' ? 'nurse' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Contraceptive Counseling & Reproductive Health Services Management
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-heart"></i>
                    Current Patient - Family Planning Consultation
                </div>
            ...
            <div class="current-patient-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete family planning consultation?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for family planning consultation.</p>
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Family Planning'); ?></p>
                                    </div>
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">Family Planning</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No FP consultations completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
