<?php
/**
 * Laboratory Station 13 Dashboard
 * 
 * Queue management dashboard for Laboratory Station 13
 * Features: Patient queue management, specimen collection tracking, visit completion
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from laboratory dashboard');
...
// Station 13 Queue Management Functions
function getStation13Queue($pdo) {
    $stmt = $pdo->prepare("
        SELECT 
...
function updateStation13QueueStatus($pdo, $queue_entry_id, $status, $employee_id, $notes = null) {
    $stmt = $pdo->prepare("
        UPDATE station_13_queue 
...
function addToStationQueue($pdo, $patient_id, $username, $visit_id, $appointment_id, $service_id, $queue_type, $station_id, $priority_level = 'normal') {
    $stmt = $pdo->prepare("
        INSERT INTO station_13_queue (patient_id, username, visit_id, appointment_id, service_id, queue_type, station_id, priority_level, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'waiting', NOW())
    ");
...
function getStation13Statistics($pdo, $date) {
    $stmt = $pdo->prepare("
        SELECT 
...
// =====================================================================
// LABORATORY STATION 13 CONFIGURATION
// =====================================================================
...
$STATION_CONFIG = [
    ...
    'allowed_roles' => ['admin', 'laboratory tech'], // Lab techs handle lab work
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // Lab can complete visits if no further action needed
...
$station = [
    ...
    'station_code' => 'LABORATORY_13',
    'description' => 'Laboratory Testing and Specimen Collection'
];
...
$currentQueue = getStation13Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation13Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation13QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to laboratory station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to laboratory successfully.';
                    } else {
            ...
            case 'complete_current':
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        if ($result['success']) {
                            $message = 'Laboratory work completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete laboratory work.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation13QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at laboratory - no further stations needed'
                        );
            ...
            case 'transfer_to_billing':
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = updateStation13QueueStatus(
                            $pdo,
                            $patient_queue['queue_entry_id'], 
                            'completed', 
                            $_SESSION['employee_id'],
                            'Laboratory work completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            ...
                            $error = 'Failed to complete laboratory work before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = updateStation13QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at laboratory'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = updateStation13QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at laboratory'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('Laboratory station dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'laboratory tech' => 'sidebar_lab_tech.php'
];
...
<head>
    ...
    <style>
        /* Laboratory Station Styles - Red Theme */
        :root {
            --station-primary: #dc2626;
            --station-secondary: #991b1b;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'laboratory tech' ? 'lab_tech' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Laboratory Services & Queue Management
                | Tech: <?php echo htmlspecialchars($_SESSION['employee_name'] ?? 'Unknown'); ?>
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    <i class="fas fa-flask"></i>
                    Current Patient - Laboratory Services
                </div>
            ...
            <div class="current-patient-content">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete laboratory work?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for laboratory services.</p>
        ...
        <div class="queue-management-grid">
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Lab Test'); ?></p>
                                    </div>
            ...
            <div class="queue-section in-progress-queue">
                <div class="queue-header">
                    <h3>
                        <i class="fas fa-user-check"></i>
                        In Laboratory (<?php echo count($inProgressQueue); ?> patients)
                    </h3>
                </div>
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">In Lab</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                        ...
                                        <div class="action-group">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-success btn-sm" 
                                                        onclick="return confirm('Complete lab work?')">
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No lab tests completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
