...
$statusFilter = isset($_GET['status']) ? $_GET['status'] : 'unpaid';  // Default to unpaid invoices
...
$invoicesSql = "SELECT b.billing_id, b.patient_id, 
          ...
          WHERE (
              -- Show today's invoices (all statuses)
              DATE(b.billing_date) = CURDATE() 
              OR 
              -- Show past invoices only if they are unpaid or partial
              (DATE(b.billing_date) < CURDATE() AND b.payment_status IN ('unpaid', 'partial'))
              OR
              -- Always show unpaid invoices regardless of date (fallback for status filter)
              b.payment_status = 'unpaid'
              OR
              -- Keep showing partial invoices for follow-up
...
<body>
    ...
    <script>
        ...
        // Reports modal function (placeholder)
        function openReportsModal() {
            alert('Reports feature will be implemented');
        }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons-table">
                                            ...
                                            <?php endif; ?>
                                            <button class="btn btn-sm btn-secondary" onclick="printInvoice(<?= $invoice['billing_id'] ?>)" title="Print Invoice">
                                                <i class="fas fa-print"></i>
                                            </button>
                                            <div class="btn-group">
                                                <button class="btn btn-sm btn-info dropdown-toggle" data-toggle="dropdown" title="Download Invoice">
                                                    <i class="fas fa-download"></i>
                                                </button>
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item" href="#" onclick="downloadInvoice(<?= $invoice['billing_id'] ?>, 'pdf')">
                                                        <i class="fas fa-file-pdf"></i> PDF
                                                    </a>
                                                    <a class="dropdown-item" href="#" onclick="downloadInvoice(<?= $invoice['billing_id'] ?>, 'html')">
    ...
    <script>
        ...
        function viewInvoice(billingId) {
            // Show loading modal
            showInvoiceModal('Loading invoice details...');
            ...
            fetch(`/wbhsms-cho-koronadal-1/api/billing/management/get_invoice_details.php?billing_id=${billingId}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        showInvoiceModal('<div class="error-message"><i class="fas fa-exclamation-triangle"></i> ' + (data.message || 'Failed to load invoice details') + '</div>');
                    }
                })
                .catch(error => {
                    console.error('Error fetching invoice:', error);
                    showInvoiceModal('<div class="error-message"><i class="fas fa-exclamation-triangle"></i> Error loading invoice details</div>');
                });
        }
        ...
        function downloadInvoice(billingId, format) {
            const downloadUrl = `/wbhsms-cho-koronadal-1/api/billing/management/download_invoice.php?billing_id=${billingId}&format=${format}`;
            ...
            if (invoice.items && invoice.items.length > 0) {
                itemsHtml = invoice.items.map(item => `
                    <tr>
                        ...
                        <td class="text-right">₱${parseFloat(item.item_price).toFixed(2)}</td>
                ...
                <div class="invoice-preview">
                    <div class="invoice-header">
                        <div class="clinic-info">
                            ...
                            <p>Email: cho.koronadal@example.com</p>
                        </div>
                        <div class="invoice-info">
                            <h3>INVOICE #${invoice.billing_id}</h3>
                            <p><strong>Date:</strong> ${new Date(invoice.billing_date).toLocaleDateString()}</p>
                            <p><strong>Status:</strong> <span class="status-badge status-${invoice.payment_status}">${invoice.payment_status.toUpperCase()}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="patient-info">
                        ...
                        <div class="patient-details">
                            <div class="detail-item">
                                ...
                                <span class="detail-value">${invoice.first_name} ${invoice.last_name}</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">Patient ID</span>
                                <span class="detail-value">${invoice.patient_number}</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">Age</span>
                                <span class="detail-value">${invoice.age || 'N/A'} years old</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">Contact</span>
                                <span class="detail-value">${invoice.contact_number || 'N/A'}</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">Address</span>
                                <span class="detail-value">${invoice.barangay_name || 'N/A'}, ${invoice.city || 'Koronadal'}</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">Visit Purpose</span>
                                <span class="detail-value">${invoice.visit_purpose || 'General Consultation'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="invoice-summary">
                        ...
                        <div class="summary-row total">
                            ...
                            <span>₱${parseFloat(invoice.net_amount).toFixed(2)}</span>
                        </div>
                        <div class="summary-row">
                            <span>Amount Paid:</span>
                            <span>₱${parseFloat(invoice.paid_amount).toFixed(2)}</span>
                        </div>
                        <div class="summary-row" style="color: ${(invoice.net_amount - invoice.paid_amount) > 0 ? '#dc3545' : '#28a745'};">
                            <span>Balance:</span>
                            <span>₱${(parseFloat(invoice.net_amount) - parseFloat(invoice.paid_amount)).toFixed(2)}</span>
                        </div>
                    </div>
