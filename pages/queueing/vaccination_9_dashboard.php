<?php
/**
 * Vaccination Station 9 Dashboard
 * 
 * Queue management dashboard for Vaccination Station 9
 * Features: Patient queue management, vaccine administration, immunization records
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from Vaccination 9 dashboard');
...
// Station 9 Queue Management Functions
function getStation9Queue($pdo) {
    $stmt = $pdo->prepare("
        SELECT 
...
function updateStation9QueueStatus($pdo, $queue_entry_id, $status, $employee_id, $notes = null) {
    $stmt = $pdo->prepare("
        UPDATE station_9_queue 
...
function addToStationQueue($pdo, $patient_id, $username, $visit_id, $appointment_id, $service_id, $queue_type, $station_id, $priority_level = 'normal') {
    $stmt = $pdo->prepare("
        INSERT INTO station_9_queue (patient_id, username, visit_id, appointment_id, service_id, queue_type, station_id, priority_level, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'waiting', NOW())
    ");
...
function getStation9Statistics($pdo, $date) {
    $stmt = $pdo->prepare("
        SELECT 
...
// =====================================================================
// VACCINATION STATION 9 CONFIGURATION
// =====================================================================

// Vaccination Station 9 Configuration
$STATION_CONFIG = [
    'station_id' => 9,                    
    'station_name' => 'Vaccination Station',  
    'station_type' => 'vaccination',          
    'icon' => 'fa-syringe',                   
    'color_scheme' => [
    ...
    'allowed_roles' => ['admin', 'doctor', 'nurse', 'vaccination_tech'], // Vaccination specialists
    'next_stations' => [                  
    ...
    'can_complete_visit' => true,         // Vaccination can complete visits for immunizations
...
$currentQueue = getStation9Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation9Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation9QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to vaccination station'
                    );
                    if ($result['success']) {
                        $message = 'Patient called to vaccination station successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        if ($result['success']) {
                            ...
                            $error = $result['message'] ?? 'Failed to complete vaccination.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation9QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at vaccination station - patient discharged'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation9QueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at vaccination station'
                        );
                        if ($result['success']) {
            ...
            case 'transfer_to_document':
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = updateStation9QueueStatus(
                            $pdo,
                            $patient_queue['queue_entry_id'], 
                            'completed', 
                            $_SESSION['employee_id'],
                            'Vaccination completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            $transfer_result = addToStationQueue(
                                ...
                                $_SESSION['username'] ?? 'VACCINATION_STAFF',
                            ...
                            $error = 'Failed to complete vaccination before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = updateStation9QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at vaccination station'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = updateStation9QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at vaccination station'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('Vaccination station 9 dashboard action error: ' . $e->getMessage());
...
$sidebar_files = [
    ...
    'vaccination_tech' => 'sidebar_nurse.php'  // Use nurse sidebar for vaccination tech
];
...
<head>
    ...
    <style>
        /* Vaccination Station 9 Styles - Orange Theme */
        :root {
            --station-primary: #ea580c;
            --station-secondary: #c2410c;
            --station-accent: #fed7aa;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="breadcrumb">
            <a href="../management/<?php echo $user_role === 'vaccination_tech' ? 'nurse' : $user_role; ?>/dashboard.php">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Vaccine Administration & Immunization Records Management
            ...
            <div class="current-patient-content">
                <?php if ($currentPatient): ?>
                    <div class="patient-info">
                        ...
                        <div class="patient-detail">
                            <strong>Vaccination Time</strong>
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Complete vaccination?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for vaccination.</p>
        ...
        <div class="queue-management-grid">
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Vaccination'); ?></p>
                                    </div>
            ...
            <div class="queue-section in-progress-queue">
                <div class="queue-header">
                    <h3>
                        <i class="fas fa-user-check"></i>
                        Being Vaccinated (<?php echo count($inProgressQueue); ?> patients)
                    </h3>
                </div>
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">Vaccination</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-syringe"></i>
                            <p>No patients being vaccinated</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        <div class="patient-cards">
                            <?php foreach (array_slice($completedQueue, -10) as $patient): // Show last 10 completed ?>
                                <div class="patient-card completed" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-completed">Vaccinated</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
