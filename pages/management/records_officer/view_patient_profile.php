<?php
// view_patient_profile.php - Records Officer Patient Profile View
...
set_error_handler(function($severity, $message, $file, $line) {
    error_log("Records Officer Patient Profile View Error [$severity]: $message in $file on line $line");
    return true; // Don't execute PHP's internal error handler
});

// Set exception handler for uncaught exceptions
set_exception_handler(function($exception) {
    error_log("Records Officer Patient Profile View Uncaught Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
// Path setup - correct for records officer folder depth
$root_path = dirname(dirname(dirname(__DIR__)));

// Load configuration first
require_once $root_path . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'env.php';

// Use employee session for records officer users
...
// Validate patient ID
if (!$patient_id) {
    error_log("Records Officer patient profile view error: No patient ID provided");
...
if (!is_numeric($patient_id) || $patient_id <= 0) {
    error_log("Records Officer patient profile view error: Invalid patient ID format: " . $patient_id);
    header('HTTP/1.1 400 Bad Request');
...
// Verify records officer is logged in - use session system
require_employee_login();

// Check if user has permission to view patient records - records officers primarily
...
try {
    ...
    foreach ($queries_to_try as $index => $query) {
        try {
            ...
            error_log("Records Officer patient profile view query error: " . $last_error);
            continue;
        }
    }

    if (!$patient_row) {
        error_log("Records Officer patient profile view error: Patient ID $patient_id not found. Last error: " . ($last_error ?? 'No errors recorded'));
        header('HTTP/1.1 404 Not Found');
        die('Error: Patient record not found.');
    }

} catch (Exception $e) {
    error_log("Error fetching patient data for records officer view: " . $e->getMessage());
...
try {
    ...
    error_log("Error fetching personal information for patient $patient_id in records officer view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching emergency contact for patient $patient_id in records officer view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lifestyle info for patient $patient_id in records officer view: " . $e->getMessage());
    $lifestyle_info = [];
}
...
try {
    ...
    error_log("Error fetching vitals in records officer view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching appointments in records officer view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching prescriptions in records officer view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lab results in records officer view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching medical history in records officer view: " . $e->getMessage());
}
...
<head>
    ...
    <title>Records Officer View - Patient Profile - <?= htmlspecialchars($patient['full_name']) ?> - WBHSMS</title>
    ...
    <style>
        ...
        /* Records Officer View Specific Styles - Hide editing elements */
        .edit-btn,
...
<body>
    <?php
    $activePage = 'patients';
    include $root_path . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'sidebar_records_officer.php';
    ?>
    ...
    <section class="homepage">
        <div class="profile-heading-bar"
            ...
            <h1 style="margin:0;font-size:2.2em;letter-spacing:1px;">
                <i class="fas fa-file-medical"></i> PATIENT PROFILE <span style="font-size: 0.65em; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 20px; vertical-align: middle;">RECORDS OFFICER VIEW</span>
            </h1>
        ...
        <!-- Records Officer View: Patient Summary Header -->
    ...
    <script>
        ...
        // Enhanced print functionality for records officer view
