...
// Use absolute path resolution - go up 2 levels from medical-records directory
...
// Define role-based permissions based on employee role
// IMPORTANT: Patient Records Management is VIEW-ONLY for most roles, but Admin retains full control
$employee_role = $_SESSION['role'];
$canEdit = in_array(strtolower($employee_role), ['admin']); // Only admin can edit/deactivate patients
$canView = true; // All authorized roles can view
$canViewArchived = in_array(strtolower($employee_role), ['admin', 'records_officer']); // Only admin and records_officer can view archived patients
$canDeactivate = in_array(strtolower($employee_role), ['admin']); // Only admin can deactivate patients
...
<head>
    ...
    <title>Patient Records Management - <?php echo ucfirst($_SESSION['role']); ?> | CHO Koronadal</title>
    <!-- CSS Files -->
    ...
    <style>
        ...
        .enhanced-dropdown .dropdown-item:hover {
            background: linear-gradient(135deg, rgba(0, 119, 182, 0.08), rgba(0, 119, 182, 0.12));
...
<body>
    ...
    <section class="homepage">
        <div class="content-wrapper" style="padding: 2rem;transition: margin-left 0.3s;">
            <div class="main-content">
                <!-- Breadcrumb Navigation -->
                <div class="breadcrumb" style="margin-top: 50px;">
                    <?php
                    // Use production-safe dashboard URLs based on role
                    $user_role = strtolower($_SESSION['role']);
                    $dashboard_url = get_role_dashboard_url($user_role);
                    ?>
                    <a href="<?php echo htmlspecialchars($dashboard_url); ?>"><i class="fas fa-home"></i> Dashboard</a>
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-3">
                        <div class="flex-grow-1" style="min-width: 300px;">
                            <div class="input-group">
                                ...
                                <input type="text" id="searchInput" class="form-control" placeholder="Search by Patient ID, First Name, Middle Name, or Last Name..." value="<?php echo htmlspecialchars($searchQuery); ?>">
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            <button id="searchBtn" class="btn btn-primary d-flex align-items-center gap-2">
                                <i class="fas fa-search"></i> Search
                            </button>
                            <button id="clearFilters" class="btn btn-secondary d-flex align-items-center gap-2">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Dropdown change handlers (immediate search)
            ...
            $(document).on('click', '#confirmDeactivate', function() {
                ...
                $.ajax({
                    url: '<?php echo defined('WBHSMS_BASE_URL') ? WBHSMS_BASE_URL . '/api/patient_deactivation.php' : '../../api/patient_deactivation.php'; ?>',
                    type: 'POST',
