    ...
    try {
        ...
        switch ($action) {
            ...
            case 'reroute_to_consultation':
                ...
                $flowValidator->logRoutingAction(
                    $queue_entry_id,
                    'route_to_consultation',
                    'billing',
                    'consultation',
                    $employee_id,
                    'Post-billing consultation (double consultation flow)'
                );
            ...
            case 'reroute_to_lab':
                ...
                if ($flowValidator->isLabOnlyService($service_id)) {
                    ...
                    $flowValidator->logRoutingAction(
                        ...
                        'lab',
                        $employee_id,
                        'Lab-only service - direct routing after billing'
                    );
                } else {
            ...
            case 'reroute_to_document':
                ...
                if ($flowValidator->isDocumentRequestService($service_id)) {
                    ...
                    $flowValidator->logRoutingAction(
                        ...
                        'document',
                        $employee_id,
                        'Document request service - routing after billing'
                    );
                } else {
    ...
    <section class="homepage">
                    ...
                    <div class="div4 card-container">
                        ...
                        <div class="section-body" style="padding: 15px 20px;">
                            <div class="actions-grid">
                                <button class="action-btn btn-success" onclick="createInvoice()">
                                    <i class="fas fa-file-invoice-dollar"></i> Create Invoice
                                </button>
                                <button class="action-btn" style="background: linear-gradient(135deg, #28a745, #20c997);" onclick="processPayment()">
        ...
        <script>
            ...
            function skipPatient() {
                ...
                if (reason !== null) {
                    performQueueAction('skip_patient', currentPatient.queue_entry_id, 'Skipping patient...', {
                        reason: reason
                    });
                }
            }
            ...
            function forceCallSpecificPatient(queueEntryId, reason = null) {
                ...
                performQueueAction('force_call', queueEntryId, 'Force calling patient...', {
                    reason: reason
                });
            }

            function createInvoice() {
                // Redirect to create invoice page with back_from parameter
                window.location.href = '../../pages/billing/create_invoice.php?back_from=billing_station';
            }

            function processPayment() {
                // Redirect to process payment page with back_from parameter
                window.location.href = '../../pages/billing/process_payment.php?back_from=billing_station';
            }
            ...
            function rerouteToConsultation() {
                ...
                performQueueAction('reroute_to_consultation', currentPatient.queue_entry_id, 'Forwarding to consultation...', {
                    remarks: remarks
                });
            }
            ...
            function rerouteToLab() {
                ...
                performQueueAction('reroute_to_lab', currentPatient.queue_entry_id, 'Forwarding to laboratory...', {
                    remarks: remarks
                });
            }
            ...
            function rerouteToDocument() {
                ...
                performQueueAction('reroute_to_document', currentPatient.queue_entry_id, 'Forwarding to document processing...', {
                    remarks: remarks
                });
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (typeof QueueSyncManager !== 'undefined') {
                    ...
                    // Create station object with required refreshData method
                    const stationObject = {
                        ...
                        updateQueue: function(queueData) {
                            // Optional: Handle real-time queue updates without full reload
                            console.log('Queue data updated for station:', stationConfig.stationId, queueData);
                        }
                    };
                    
                    window.queueSync.registerStation(stationConfig.stationType, stationConfig.stationId, stationObject);
