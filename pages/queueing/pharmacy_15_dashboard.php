<?php
/**
 * Pharmacy Station 15 Dashboard
 * 
 * Queue management dashboard for Pharmacy Station 15
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from pharmacy 15 dashboard');
...
// Station 15 Queue Management Functions
function getStation15Queue($pdo) {
    $stmt = $pdo->prepare("
        SELECT 
...
function updateStation15QueueStatus($pdo, $queue_entry_id, $status, $employee_id, $notes = null) {
    $stmt = $pdo->prepare("
        UPDATE station_15_queue 
...
function addToStationQueue($pdo, $patient_id, $username, $visit_id, $appointment_id, $service_id, $queue_type, $station_id, $priority_level = 'normal') {
    $stmt = $pdo->prepare("
        INSERT INTO station_15_queue (patient_id, username, visit_id, appointment_id, service_id, queue_type, station_id, priority_level, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'waiting', NOW())
    ");
...
function getStation15Statistics($pdo, $date) {
    $stmt = $pdo->prepare("
        SELECT 
...
// =====================================================================
// PHARMACY STATION 15 CONFIGURATION
// =====================================================================

// Pharmacy Station 15 Configuration
$STATION_CONFIG = [
    'station_id' => 15,                    
    'station_name' => 'Pharmacy 2',        
...
$station = [
    ...
    'station_code' => 'PHARMACY_15',
    'description' => 'Secondary Pharmacy - Medication Dispensing'
];
...
$currentQueue = getStation15Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation15Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation15QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to pharmacy 2'
                    );
                    if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation15QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Medication dispensing completed at pharmacy 2'
                        );
                        if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = updateStation15QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at pharmacy 2 - patient discharged'
                        );
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at pharmacy 2'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at pharmacy 2'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        error_log('Pharmacy station 15 dashboard action error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        /* Pharmacy Station 15 Styles - Purple Theme */
        :root {
