<?php
/**
 * Triage Station 3 Dashboard
 * 
 * Specialized dashboard for Triage Station 3 operations
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from triage 3 dashboard');
...
// Station 3 Queue Management Functions
function getStation3Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation3Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation3QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation3QueueStatus Error: ' . $e->getMessage());
...
function getStation3Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation3Statistics Error: ' . $e->getMessage());
        return ['total_served' => 0, 'completed' => 0, 'waiting' => 0, 'in_progress' => 0, 'avg_wait_time' => 0];
    }
}

// =====================================================================
// TRIAGE STATION 3 CONFIGURATION
// =====================================================================

// Triage Station 3 Configuration
$STATION_CONFIG = [
    'station_id' => 3,                    
    'station_name' => 'Triage Station 3', 
    'station_type' => 'triage',           
    'icon' => 'fa-stethoscope',           
    'color_scheme' => [
        'primary' => '#059669',           // Emerald green (different from Stations 1 & 2)
        'secondary' => '#047857',         
        'accent' => '#6ee7b7'             
    ],
...
// Get station details directly - Station 3 is always Triage 3
$station_id = $STATION_CONFIG['station_id'];
$station = [
    'id' => 3,
    'name' => 'Triage Station 3',
    'type' => 'triage',
    'description' => 'Tertiary triage and patient assessment'
];
...
// Get current queue and statistics based on station_3_queue table
$currentQueue = getStation3Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation3Statistics($pdo, $today);
...
// Process queue actions (same as other triage stations)
$message = '';
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
            ...
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'Patient called to triage station 3'
                    );
                    if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Triage assessment completed at station 3'
                        );
                        if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at triage station 3'
                        );
                        if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Transferred to ' . $target_name . ' from triage station 3'
                        );
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled to triage station 3'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at station 3'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        // Refresh data after action using station_3_queue
        $currentQueue = getStation3Queue($pdo);
        $stationStats = getStation3Statistics($pdo, $today);
        ...
        error_log('Triage station 3 dashboard action error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        /* Triage Station 3 Styles - Emerald Green Theme */
        :root {
            --station-primary: #059669;
            --station-secondary: #047857;
            --station-accent: #6ee7b7;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
    ...
    <div class="content-wrapper">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Triage Assessment & Patient Routing (Station 3)
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    ...
                    Current Patient - Triage Assessment (Station 3)
                </div>
            ...
            <div class="current-patient-content">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for triage assessment at Station 3.</p>
