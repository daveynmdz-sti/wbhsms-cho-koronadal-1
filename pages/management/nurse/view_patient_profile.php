<?php
// view_patient_profile.php - Nurse Patient Profile View
...
set_error_handler(function($severity, $message, $file, $line) {
    error_log("Nurse Patient Profile View Error [$severity]: $message in $file on line $line");
    return true; // Don't execute PHP's internal error handler
});

// Set exception handler for uncaught exceptions
set_exception_handler(function($exception) {
    error_log("Nurse Patient Profile View Uncaught Exception: " . $exception->getMessage() . " in " . $exception->getFile() . " on line " . $exception->getLine());
...
// Validate patient ID
if (!$patient_id) {
    error_log("Nurse patient profile view error: No patient ID provided");
...
if (!is_numeric($patient_id) || $patient_id <= 0) {
    error_log("Nurse patient profile view error: Invalid patient ID format: " . $patient_id);
    header('HTTP/1.1 400 Bad Request');
...
// Verify nurse is logged in - use session system
require_employee_login();

// Check if user has permission to view patient records - nurses and doctors
...
try {
    ...
    foreach ($queries_to_try as $index => $query) {
        try {
            ...
            error_log("Nurse patient profile view query error: " . $last_error);
            continue;
        }
    }

    if (!$patient_row) {
        error_log("Nurse patient profile view error: Patient ID $patient_id not found. Last error: " . ($last_error ?? 'No errors recorded'));
        header('HTTP/1.1 404 Not Found');
        die('Error: Patient record not found.');
    }

} catch (Exception $e) {
    error_log("Error fetching patient data for nurse view: " . $e->getMessage());
...
try {
    ...
    error_log("Error fetching personal information for patient $patient_id in nurse view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching emergency contact for patient $patient_id in nurse view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lifestyle info for patient $patient_id in nurse view: " . $e->getMessage());
    $lifestyle_info = [];
}
...
try {
    ...
    error_log("Error fetching vitals in nurse view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching appointments in nurse view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching prescriptions in nurse view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching lab results in nurse view: " . $e->getMessage());
}
...
try {
    ...
    error_log("Error fetching medical history in nurse view: " . $e->getMessage());
}
...
<head>
    <meta charset="UTF-8" />
    <title>Nurse View - Patient Profile - <?= htmlspecialchars($patient['full_name']) ?> - WBHSMS</title>
    ...
    <style>
        ...
        /* Nurse View Specific Styles - Hide editing elements */
        .edit-btn,
...
<body>
    <?php
    $activePage = 'patients';
    include $root_path . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'sidebar_nurse.php';
    ?>
    ...
    <section class="homepage">
        ...
        <!-- Nurse View: Patient Summary Header -->
    ...
    <script>
        ...
        // Enhanced print functionality for nurse view
