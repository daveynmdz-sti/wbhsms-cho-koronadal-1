<?php
/**
 * Primary Care Station 5 Dashboard
 * 
 * Specialized dashboard for Primary Care Station 5 operations
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from primary care 5 dashboard');
...
// Station 5 Queue Management Functions
function getStation5Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation5Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation5QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation5QueueStatus Error: ' . $e->getMessage());
...
function getStation5Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation5Statistics Error: ' . $e->getMessage());
        return ['total_served' => 0, 'completed' => 0, 'waiting' => 0, 'in_progress' => 0, 'avg_wait_time' => 0];
    }
}

// =====================================================================
// PRIMARY CARE STATION 5 CONFIGURATION
// =====================================================================
...
// Get station details directly - Station 5 is always Primary Care 1
$station_id = $STATION_CONFIG['station_id'];
$station = [
    'id' => 5,
    'name' => 'Primary Care 1',
    'type' => 'consultation',
    'description' => 'Primary care consultation and clinical services'
];
...
// Get current queue and statistics based on station_5_queue table
$currentQueue = getStation5Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation5Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation5QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called for primary care consultation'
                    );
                    if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = updateStation5QueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Consultation completed at Primary Care 1'
                        );
                        if ($result['success']) {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at Primary Care 1 - no further stations needed'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at Primary Care 1'
                        );
                        if ($result['success']) {
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at Primary Care 1'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at Primary Care 1'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        // Refresh data after action using station_5_queue
        $currentQueue = getStation5Queue($pdo);
        $stationStats = getStation5Statistics($pdo, $today);
        ...
        error_log('Primary care station 5 dashboard action error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        /* Primary Care Station 5 Styles - Blue Theme */
        :root {
            --station-primary: #3b82f6;
            --station-secondary: #1e40af;
            --station-accent: #93c5fd;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
    ...
    <div class="content-wrapper">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Medical Consultations & Patient Care
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    ...
                    Current Patient - Medical Consultation
                </div>
