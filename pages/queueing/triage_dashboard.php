<?php
/**
 * Triage Station 1 Dashboard
 * 
 * Specialized dashboard for Triage Station operations
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from triage dashboard');
...
// Triage Station 1 Configuration
$STATION_CONFIG = [
    'station_id' => 1,                    
...
// Station 1 Queue Management Functions
function getStation1Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation1Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation1QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation1QueueStatus Error: ' . $e->getMessage());
...
function getStation1Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation1Statistics Error: ' . $e->getMessage());
...
// Get station details directly - Station 1 is always Triage
$station = [
    'id' => 1,
    'name' => 'Triage',
    'type' => 'triage',
    'description' => 'Primary patient triage and initial assessment'
];
...
// Get current queue and statistics based on station_1_queue table
$currentQueue = getStation1Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation1Statistics($pdo, $today);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation1QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called to triage station'
                    );
                    if ($result['success']) {
            ...
            case 'transfer_to_consultation':
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $complete_result = updateStation1QueueStatus(
                            $pdo,
                            $patient_queue['queue_entry_id'], 
                            'completed', 
                            $_SESSION['employee_id'],
                            'Transferred to consultation from triage'
                        );
                        
                        if ($complete_result['success']) {
                            // Create new queue entry at consultation station (Station 5)
                            $transfer_result = addToStationQueue(
                                $pdo,
                                5, // Station 5 - Primary Care
                            ...
                            if ($transfer_result['success']) {
                                $message = 'Patient transferred to consultation successfully.';
                            } else {
                                $error = 'Failed to create queue entry at consultation station.';
                            }
                        } else {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = updateStation1QueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status'
                    );
                    if ($result['success']) {
        ...
        // Refresh data after action using station_1_queue
        $currentQueue = getStation1Queue($pdo);
        $stationStats = getStation1Statistics($pdo, $today);
        ...
        error_log('Triage dashboard action error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        /* Triage Station Styles */
        :root {
            --station-primary: #2563eb;
            --station-secondary: #1e40af;
            --station-accent: #60a5fa;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Triage Assessment & Patient Routing 
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    ...
                    Current Patient - Triage Assessment
                </div>
            ...
            <div class="current-patient-content">
                <?php if ($currentPatient): ?>
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            <input type="hidden" name="action" value="transfer_to_consultation">
                            <input type="hidden" name="patient_id" value="<?php echo $currentPatient['patient_id']; ?>">
                            <button type="submit" class="btn btn-primary" 
                                    onclick="return confirm('Send patient to consultation?')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for triage assessment.</p>
                                                ...
                                                <div class="transfer-menu">
                                                    <?php foreach ($STATION_CONFIG['next_stations'] as $key => $station): ?>
                                                        <form method="POST" style="display: block;">
                                                            ...
                                                            <input type="hidden" name="patient_id" value="<?php echo $patient['patient_id']; ?>">
                                                            <input type="hidden" name="target_station" value="<?php echo $station['station_id']; ?>">
                                                            <button type="submit" class="transfer-option" 
                                                                    onclick="return confirm('Transfer to <?php echo $station['name']; ?>?')">
    ...
    <script>
        ...
        // Real-time updates for queue counts
        function updateQueueCounts() {
            // This would typically use AJAX to get updated counts without full page refresh
            // For now, we rely on the auto-refresh mechanism
        }
