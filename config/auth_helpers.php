 ...
 * Provides production-ready redirect handling and enhanced security
 * Works with existing employee_session.php functions
 */
 ...
 * @return bool
 */
if (!function_exists('check_employee_auth')) {
    function check_employee_auth($allowed_roles = [], $redirect = true) {
        // Check if user is logged in
        if (!is_employee_logged_in()) {
            if ($redirect) {
                redirect_to_employee_login(null, 'auth');
            }
            return false;
        }
 ...
 * @param array $allowed_roles Array of allowed roles
 */
if (!function_exists('require_employee_auth')) {
 ...
 */
if (!function_exists('handle_session_security')) {
    function handle_session_security() {
        ...
        if (is_employee_logged_in()) {
            ...
            if ($stored_ip && $stored_ip !== $current_ip) {
                ...
                redirect_to_employee_login(null, 'security');
            }
            ...
            // Update session security info using existing function
            if (function_exists('set_employee_session')) {
 ...
 * Uses existing employee_session.php functions
 */
if (!function_exists('check_session_timeout')) {
    function check_session_timeout() {
        if (!is_employee_logged_in()) {
            redirect_to_employee_login(null, 'timeout');
            return;
        }
        ...
        if ($last_activity && (time() - $last_activity > $timeout)) {
            ...
            // Update last activity using existing function
            if (function_exists('set_employee_session')) {
