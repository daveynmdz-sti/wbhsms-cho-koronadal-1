<?php
/**
 * Primary Care Station 6 Dashboard
 * 
 * Specialized dashboard for Primary Care Station 6 operations
 * Features: Patient consultations, clinical notes, prescriptions, lab orders
 */
...
if (!isset($_SESSION['employee_id']) || !isset($_SESSION['role'])) {
    ...
    error_log('Redirecting to employee_login from primary care 6 dashboard');
...
// Station 6 Queue Management Functions
function getStation6Queue($pdo) {
    try {
        $stmt = $pdo->prepare("
            SELECT q.*, 
        ...
        error_log('getStation6Queue Error: ' . $e->getMessage());
        return [];
    }
}

function updateStation6QueueStatus($pdo, $queue_entry_id, $status, $employee_id = null, $remarks = null) {
    try {
        $time_field = '';
        switch ($status) {
            case 'in_progress':
        ...
        error_log('updateStation6QueueStatus Error: ' . $e->getMessage());
...
function getStation6Statistics($pdo, $date = null) {
    try {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        error_log('getStation6Statistics Error: ' . $e->getMessage());
        return ['total_served' => 0, 'completed' => 0, 'waiting' => 0, 'in_progress' => 0, 'avg_wait_time' => 0];
    }
}

// =====================================================================
// PRIMARY CARE STATION 6 CONFIGURATION
// =====================================================================
...
$STATION_CONFIG = [
    ...
    'icon' => 'fa-user-md',               
    'color_scheme' => [
        'primary' => '#7c3aed',           // Purple primary (different from Station 5)
        'secondary' => '#5b21b6',         
        'accent' => '#c4b5fd'             
    ],
    'allowed_roles' => ['admin', 'doctor'], // Doctors primarily handle consultations
    'next_stations' => [                  
        'laboratory' => ['station_id' => 13, 'name' => 'Laboratory', 'icon' => 'fa-flask'],
        'pharmacy' => ['station_id' => 14, 'name' => 'Pharmacy 1', 'icon' => 'fa-pills'],
        'pharmacy2' => ['station_id' => 15, 'name' => 'Pharmacy 2', 'icon' => 'fa-pills'],
        'billing' => ['station_id' => 4, 'name' => 'Billing', 'icon' => 'fa-receipt']
    ],
    'can_complete_visit' => true,         // Consultations can complete visits if no further action needed
    'special_functions' => [              
        'clinical_notes' => true,         // Essential for consultations
        'prescriptions' => true,          
        'lab_orders' => true,
        'view_history' => true,
...
// Get station details directly - Station 6 is always Primary Care 2
$station_id = $STATION_CONFIG['station_id'];
$station = [
    'id' => 6,
    'name' => 'Primary Care 2',
    'type' => 'consultation',
    'description' => 'Secondary primary care consultation and clinical services'
];
...
// Get current queue and statistics based on station_6_queue table
$currentQueue = getStation6Queue($pdo);
$today = date('Y-m-d');
$stationStats = getStation6Statistics($pdo, $today);
...
// Process queue actions (same as Station 5 but with Station 6 identifiers)
$message = '';
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'call_next':
                ...
                if (!empty($waiting_patients)) {
                    ...
                    $result = updateStation6QueueStatus(
                        $pdo,
                        $next_patient['queue_entry_id'], 
                        'in_progress', 
                        $_SESSION['employee_id'],
                        'Patient called for primary care consultation (Station 6)'
                    );
                    if ($result['success']) {
                        $message = 'Patient called for consultation successfully.';
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Consultation completed at Primary Care 2'
                        );
                        if ($result['success']) {
                            $message = 'Consultation completed successfully.';
                        } else {
                            $error = $result['message'] ?? 'Failed to complete consultation.';
                        }
                    } else {
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        ...
                        $result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Visit completed at Primary Care 2 - no further stations needed'
                        );
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $result = $queueService->updateQueueStatus(
                            ...
                            'skipped', 
                            $_SESSION['employee_id'],
                            'Patient skipped at Primary Care 2'
                        );
                        if ($result['success']) {
            ...
            case 'transfer_to_laboratory':
            case 'transfer_to_pharmacy':
            case 'transfer_to_pharmacy2':
            case 'transfer_to_billing':
                $transfer_map = [
                    'transfer_to_laboratory' => ['id' => 13, 'name' => 'Laboratory', 'type' => 'lab'],
                    'transfer_to_pharmacy' => ['id' => 14, 'name' => 'Pharmacy 1', 'type' => 'pharmacy'],
                    'transfer_to_pharmacy2' => ['id' => 15, 'name' => 'Pharmacy 2', 'type' => 'pharmacy'],
                ...
                if ($patient_id) {
                    ...
                    if ($patient_queue) {
                        $complete_result = $queueService->updateQueueStatus(
                            ...
                            'completed', 
                            $_SESSION['employee_id'],
                            'Consultation completed, transferred to ' . $target_name
                        );
                        ...
                        if ($complete_result['success']) {
                            ...
                            $error = 'Failed to complete consultation before transfer.';
                        }
                    } else {
                        $error = 'Patient not found in active queue.';
                    }
            ...
            case 'recall_patient':
                ...
                if (!empty($skipped_patients)) {
                    ...
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled at Primary Care 2'
                    );
                    if ($result['success']) {
            ...
            case 'recall_specific_patient':
                ...
                if ($queue_entry_id) {
                    $result = $queueService->updateQueueStatus(
                        ...
                        'waiting', 
                        $_SESSION['employee_id'],
                        'Patient recalled from skipped status at Primary Care 2'
                    );
                    if ($result['success']) {
                        $message = 'Patient recalled successfully.';
                    } else {
        ...
        // Refresh data after action using station_6_queue
        $currentQueue = getStation6Queue($pdo);
        $stationStats = getStation6Statistics($pdo, $today);
        ...
        error_log('Primary care station 6 dashboard action error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        /* Primary Care Station 6 Styles - Purple Theme */
        :root {
            --station-primary: #7c3aed;
            --station-secondary: #5b21b6;
            --station-accent: #c4b5fd;
            --success: #10b981;
            --warning: #f59e0b;
            --danger: #ef4444;
            --info: #3b82f6;
        ...
        .clinical-btn {
            background: linear-gradient(45deg, var(--station-primary), var(--station-secondary));
            color: white;
            border: none;
            padding: 0.75rem 1rem;
            border-radius: var(--border-radius);
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="station-header">
            ...
            <div class="station-info">
                Medical Consultations & Patient Care (Secondary Station)
                | Doctor: <?php echo htmlspecialchars($_SESSION['employee_name'] ?? 'Unknown'); ?>
        ...
        <div class="current-patient">
            <div class="current-patient-header">
                <div>
                    ...
                    Current Patient - Medical Consultation (Station 2)
                </div>
            ...
            <div class="current-patient-content">
                <?php if ($currentPatient): ?>
                    ...
                    <div class="clinical-features">
                        <button type="button" class="clinical-btn" onclick="openClinicalNotes(<?php echo $currentPatient['patient_id']; ?>)">
                            <i class="fas fa-notes-medical"></i>
                            Clinical Notes
                        </button>
                        <button type="button" class="clinical-btn" onclick="createPrescription(<?php echo $currentPatient['patient_id']; ?>)">
                            <i class="fas fa-prescription-bottle"></i>
                            Prescriptions
                        </button>
                        <button type="button" class="clinical-btn" onclick="orderLabTests(<?php echo $currentPatient['patient_id']; ?>)">
                            <i class="fas fa-flask"></i>
                            Lab Orders
                        </button>
                        <button type="button" class="clinical-btn" onclick="viewPatientHistory(<?php echo $currentPatient['patient_id']; ?>)">
                    ...
                    <div class="action-buttons">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-success" 
                                    onclick="return confirm('Mark consultation as complete?')">
                        ...
                        <form method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn btn-info" 
                                    onclick="return confirm('Complete entire visit? Patient will not go to other stations.')">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Call the next patient for medical consultation.</p>
        ...
        <div class="queue-management-grid">
                            ...
                            <?php foreach ($waitingQueue as $index => $patient): ?>
                                <div class="patient-card waiting" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    ...
                                    <div class="patient-card-content">
                                        ...
                                        <p>Type: <?php echo htmlspecialchars($patient['visit_type'] ?? 'Consultation'); ?></p>
                                    </div>
            ...
            <div class="queue-section in-progress-queue">
                <div class="queue-header">
                    <h3>
                        <i class="fas fa-user-check"></i>
                        In Consultation (<?php echo count($inProgressQueue); ?> patients)
                    </h3>
                </div>
                <div class="queue-content">
                        ...
                        <div class="patient-cards">
                            <?php foreach ($inProgressQueue as $patient): ?>
                                <div class="patient-card in-progress" data-patient-id="<?php echo $patient['patient_id']; ?>">
                                    <div class="patient-card-header">
                                        <div class="queue-number"><?php echo htmlspecialchars($patient['queue_number']); ?></div>
                                        <span class="status-badge-queue status-in-progress">In Consultation</span>
                                    </div>
                                    <div class="patient-card-content">
                                        <h4><?php echo htmlspecialchars($patient['patient_name']); ?></h4>
                                    ...
                                    <div class="patient-card-actions">
                                        <div class="action-group">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-success btn-sm" 
                                                        onclick="return confirm('Complete consultation?')">
                                            ...
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" class="btn btn-info btn-sm" 
                                                        onclick="return confirm('Complete entire visit?')">
                                        ...
                                        <div class="special-actions">
                                            <button type="button" class="btn btn-info btn-sm" 
                                                    onclick="openClinicalNotes(<?php echo $patient['patient_id']; ?>)">
                                                <i class="fas fa-notes-medical"></i> Notes
                                            </button>
                                            <button type="button" class="btn btn-secondary btn-sm" 
                                                    onclick="createPrescription(<?php echo $patient['patient_id']; ?>)">
            ...
            <div class="queue-section completed-queue">
                ...
                <div class="queue-content">
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            <i class="fas fa-check-circle"></i>
                            <p>No consultations completed yet</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <script>
        ...
        // Clinical functions
        function openClinicalNotes(patientId) {
            window.open(`../clinical/clinical_notes.php?patient_id=${patientId}`, '_blank');
        }

        function createPrescription(patientId) {
            window.open(`../clinical/create_prescription.php?patient_id=${patientId}`, '_blank');
        }

        function orderLabTests(patientId) {
            window.open(`../clinical/lab_orders.php?patient_id=${patientId}`, '_blank');
        }
